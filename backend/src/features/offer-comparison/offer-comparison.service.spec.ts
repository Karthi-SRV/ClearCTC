...
function validAiResponse(companyNames: string[]) {
  return {
    offers: companyNames.map((name, i) => ({
      ...
      riskAssessment: {
        level: 'low',
        factors: ['Established company'],
        benefitForRisk: 'Savings adequate',
      },
      score: 70 + i * 5,
      scoreBreakdown: {
        financial: 28 + i * 2,
    ...
    recommendation: {
      ...
      whyNotOthers: Object.fromEntries(
        companyNames.slice(0, -1).map((n) => [n, 'Lower score.']),
      ),
      confidence: 'high',
...
function makeService(
  ...
  const comp = overrides.comp ?? {
    computeOfferSnapshot: jest.fn(
      (input: any, breakdown: any, col: number) => ({
        ...
        isWfh: input.isWfh,
        variableAnnual: 250_000,
        fixedPayAnnual: 2_000_000,
        basicAnnual: 1_000_000,
        basicMonthly: 83_333,
        employeePfMonthly: 1_800,
        employeePfAnnual: 21_600,
        employerPfAnnual: 21_600,
        gratuityAccrualAnnual: 48_077,
        effectiveCtcAnnual: 2_250_000,
        taxableIncome: 2_175_000,
        incomeTaxAnnual: 234_000,
        monthlyInHand: 166_200,
        annualInHand: 1_994_400,
        colIndexUsed: col,
        monthlyExpenses: breakdown.total,
        monthlySavings: 106_200,
        annualSavings: 1_274_400,
  ...
  return new OfferComparisonService(
    comp,
    data,
    cityExpenseService,
    userModel,
...
describe('OfferComparisonService.validateAiResponse', () => {
  ...
  it('throws AiParseError when score is out of range (>100)', () => {
    ...
    expect(() => svc.validateAiResponse(raw, ['Acme', 'Beta'])).toThrow(
      AiParseError,
    );
  });
...
describe('OfferComparisonService.detectStale', () => {
  ...
  it('91 days ago → stale', () => {
    expect(svc.detectStale(daysAgo(91))).toBe(true);
  });
  it('89 days ago → not stale', () => {
    expect(svc.detectStale(daysAgo(89))).toBe(false);
  });
  it('today → not stale', () => {
    expect(svc.detectStale(new Date())).toBe(false);
  });
});
...
describe('OfferComparisonService.execute', () => {
  it('makes exactly ONE AI call regardless of offer count', async () => {
    const aiMock = {
      call: jest.fn().mockResolvedValue(validAiResponse(['Acme', 'Beta'])),
    };
  ...
  it('makes exactly one AI call for three offers too', async () => {
    const aiMock = {
      call: jest.fn().mockResolvedValue(validAiResponse(['A', 'B', 'C'])),
    };
    const svc = makeService({ ai: aiMock });
      ...
      computeOfferSnapshot: jest.fn(
        (input: any, breakdown: any, col: number) => {
          ...
          return {
            ...
            employerPfAnnual: 21_600,
            gratuityAccrualAnnual: 48_077,
            effectiveCtcAnnual: 2_000_000,
            taxableIncome: 1_925_000,
            incomeTaxAnnual: 200_000,
            monthlyInHand: 148_200,
            annualInHand: 1_778_400,
            colIndexUsed: col,
            monthlyExpenses: breakdown.total,
            monthlySavings: 88_200,
            annualSavings: 1_058_400,
            expenseBreakdown: breakdown,
            computedAt: '2025-01-01T00:00:00.000Z',
          };
        },
  ...
  it('throws NotFoundException when user is not found', async () => {
    ...
    await expect(
      svc.execute('missing', { offers: [makeOffer('A'), makeOffer('B')] }),
  ...
  it('propagates AiParseError from validateAiResponse', async () => {
    ...
    await expect(
      svc.execute('user1', { offers: [makeOffer('Acme'), makeOffer('Beta')] }),
