...
describe('CachedDataSource.getCOLIndex', () => {
  let ds: CachedDataSource;

  beforeEach(() => {
    ds = new CachedDataSource(makeMockModel(null));
  });

  it('"Bangalore" → 100', async () => {
    expect(await ds.getCOLIndex('Bangalore')).toBe(100);
  });

  it('"Mumbai" → 115', async () => {
    expect(await ds.getCOLIndex('Mumbai')).toBe(115);
  });
...
describe('CachedDataSource.getCompany', () => {
  it('exact name match returns record', async () => {
    const ds = new CachedDataSource(makeMockModel(SEED_COMPANY));
  ...
  it('unknown name returns null', async () => {
    const ds = new CachedDataSource(makeMockModel(null));
