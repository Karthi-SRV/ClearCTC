...
export class CityExpenseCacheService implements OnModuleDestroy {
  ...
  async get(city: string): Promise<CityExpense | null> {
    try {
      ...
      this.logger.warn(`Redis GET failed for ${city}: ${(err as Error).message}`);
      return null;
    }
  }
  ...
  async set(city: string, data: CityExpense): Promise<void> {
    try {
      await this.redis.set(CACHE_KEY(city), JSON.stringify(data), 'EX', TTL_SECONDS);
    } catch (err) {
      this.logger.warn(`Redis SET failed for ${city}: ${(err as Error).message}`);
    }
  }
  ...
  async del(city: string): Promise<void> {
    try {
      ...
      this.logger.warn(`Redis DEL failed for ${city}: ${(err as Error).message}`);
    }
  }
}
