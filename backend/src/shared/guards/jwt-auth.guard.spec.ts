...
describe('JwtAuthGuard', () => {
  ...
  it('throws UnauthorizedException when no Authorization header', () => {
    const { reflector, config, ctx } = makeCtx({ isPublic: false, authHeader: '' });
    const guard = new JwtAuthGuard(
      { verify: jest.fn() } as never,
      reflector as never,
      config as never,
  ...
  it('throws UnauthorizedException for non-Bearer scheme', () => {
    const { reflector, config, ctx } = makeCtx({ isPublic: false, authHeader: 'Basic abc123' });
    const guard = new JwtAuthGuard(
      { verify: jest.fn() } as never,
      reflector as never,
      config as never,
  ...
  it('returns true and sets request.user when token is valid', () => {
    ...
    const guard = new JwtAuthGuard(jwtMock as never, reflector as never, config as never);

    expect(guard.canActivate(ctx as never)).toBe(true);
    expect(request.user).toEqual(payload);
    expect(jwtMock.verify).toHaveBeenCalledWith('valid-token', { secret: 'test-secret' });
  });

  it('throws UnauthorizedException when jwt.verify throws', () => {
    const jwtMock = { verify: jest.fn().mockImplementation(() => { throw new Error('expired'); }) };
