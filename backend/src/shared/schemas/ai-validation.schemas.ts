...
export const AiOfferEntrySchema = z
  .object({
  ...
  .refine(
    (o) => {
      const sum =
        (o.scoreBreakdown.financial || 0) +
        (o.scoreBreakdown.qualitative || 0) +
        (o.scoreBreakdown.risk || 0);
...
export type CompanyAiProfileValidationResult = z.infer<
  typeof CompanyAiProfileSchema
>;

export const CompanyFetchRoleSchema = z
  .object({
...
export const CompanyFetchSchema = z.object({
  ...
  roles: z
    .array(CompanyFetchRoleSchema)
    .min(1, 'roles must contain at least 1 role'),
  ratings: z
    .array(CompanyFetchRatingSchema)
    .min(1, 'ratings must contain at least 1 rating'),
  reviews: z
    .array(CompanyFetchReviewSchema)
    .min(1, 'reviews must contain at least 1 review'),
});
