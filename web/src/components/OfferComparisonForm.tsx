...
interface OfferDraft {
  companyName: string;
  totalCtcLpa: number;          // 5–200
  variablePct: number;          // 0–100 %
  variableGuaranteed: boolean;
  joiningBonusLpa: number;      // optional LPA
...
interface Props {
  onResult: (res: Phase2Response) => void;
}
  ...
  return (
    <div className="p2-preview">
      <div className="p2-preview__row">
        ...
        <span>{INR.format(preview.variableAnnual)} /yr
  ...
  return (
    <div className="p2-offer-card" style={{ '--offer-color': accent } as React.CSSProperties}>
      <div className="p2-offer-card__header">
        ...
        {canRemove && (
          <button type="button" className="p2-offer-card__remove" onClick={onRemove} aria-label={`Remove offer ${index + 1}`}>✕</button>
        )}
      </div>
      ...
      <div className="p2-offer-card__grid">
        ...
        <div className="p2-field">
          ...
          <input
            ...
            onChange={e => onChange({ totalCtcLpa: Math.max(5, parseFloat(e.target.value) || 5) })}
          />
        </div>
        ...
        {/* Joining bonus */}
        <div className="p2-field">
          <label className="p2-field__label">Joining bonus (one-time)</label>
          <input
            className="p2-field__input"
        ...
        <div className="p2-field">
          ...
          <div className="p2-radio-group">
            <label className="p2-radio">
              <input type="radio" name={`wfh-${index}`} checked={!offer.isWfh} onChange={() => onChange({ isWfh: false })} />
              Office
            </label>
            <label className="p2-radio">
              <input type="radio" name={`wfh-${index}`} checked={offer.isWfh} onChange={() => onChange({ isWfh: true })} />
              WFH
            </label>
          </div>
        </div>

        {/* Target city (shown only when Office) */}
        {!offer.isWfh && (
          <div className="p2-field">
            <label className="p2-field__label">
              Target city
...
// ── Form root ─────────────────────────────────────────────────────────────────
export default function Phase2Form({ onResult }: Props) {
  const { user } = useAuth();
  ...
  const addOffer = useCallback(() => {
    setOffers(prev => prev.length < 10 ? [...prev, emptyOffer()] : prev);
  }, []);

  const removeOffer = useCallback((i: number) => {
    setOffers(prev => prev.length > 2 ? prev.filter((_, idx) => idx !== i) : prev);
  }, []);

  // Stable per-offer handlers — only recreated when offer count changes
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const isFamily = householdSize !== 'individual';
      const memberCount = isFamily ? parseInt(householdSize.replace('family', ''), 10) : 1;

      const res = await apiFetch(OFFER_COMPARISONS, {
        method: 'POST',
        body: JSON.stringify({
          offers: completeOffers.map(o => ({
            ...
            targetCity: o.isWfh ? user!.currentCity : o.targetCity.trim(),
            isWfh: o.isWfh,
          })),
          familyType: isFamily ? 'family' : 'individual',
      ...
      const data: Phase2Response = await res.json();
      onResult(data);
    } catch (err) {
  ...
  }, [offers, householdSize, apiFetch, onResult, user]);
  ...
  return (
    <form className="p2-form" onSubmit={handleSubmit} noValidate>
      {/* Household selector */}
      <div className="ce-household-selector" style={{ marginBottom: '24px' }}>
        <label htmlFor="household-size-select-oc">Household Size:</label>
        <select
          id="household-size-select-oc"
          value={householdSize}
